...
# Node installation function for multiple wallets (50 nodes)
function install_multiple_nodes() {
    ...
    # Check if there are at least 50 addresses and private keys
    if [ ${#addresses[@]} -lt 50 ] || [ ${#private_keys[@]} -lt 50 ]; then
        echo "There must be at least 50 entries in both address.txt and private_keys.txt."
        exit 1
    fi

    # Loop through and install 50 nodes
    for i in {1..50}; do
...
function main_menu() {
    ...
    echo "4. Install 50 Elixir V3 Nodes from private_keys.txt"
